...
| `api/main.py` | 修改 | 注册 Google Maps 专属 API 与 `/google-maps` 页面；不在通用平台列表展示 Google Maps |
...
| 页面 | 地址 | 用途 | 注意事项 |
| --- | --- | --- | --- |
| MediaCrawler 原始主页 | `http://127.0.0.1:8080/` | 原项目通用控制台 | 不展示 Google Maps，避免使用不完整的通用表单启动点位采集 |
| Google Maps 专属页 | `http://127.0.0.1:8080/google-maps` | Google Maps 点位采集控制台 | 推荐使用，支持任务预览、启动、停止、状态、报告和导出 |

Google Maps 插件完整操作入口是 `/google-maps`。原始 React WebUI 使用通用采集表单，没有 Google Maps 点位文件、任务预览、报告和导出等专属能力，因此不作为 Google Maps 的启动入口。
