...
def test_google_maps_start_rejects_file_save_options():
    app = FastAPI()
    app.include_router(crawler_router)
    client = TestClient(app)

    for save_option in ["jsonl", "csv", "json"]:
        response = client.post(
            "/crawler/start",
            json={
                "platform": "google_maps",
...
def test_google_maps_is_not_exposed_in_generic_platform_options():
    client = TestClient(api_app)

    response = client.get("/api/config/platforms")

    assert response.status_code == 200
    platform_values = {item["value"] for item in response.json()["platforms"]}
    assert "google_maps" not in platform_values
