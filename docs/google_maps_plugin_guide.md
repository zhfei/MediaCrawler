...
| 语言区域 | `pt-BR` |
| 缩放级别 | `20z` |
| 存储方式 | CLI 保留 `jsonl`、`json`、`csv`、`sqlite`、`db/postgres` 存储实现；WebUI/API 启动当前只开放 `sqlite`，用于保证任务状态、断点续采、报告和导出数据源一致 |
...
Google Maps 不在原始首页 `http://127.0.0.1:8080/` 的通用平台下拉框中展示。请使用 `/google-maps` 专属控制台完成点位文件选择、任务预览、采集启动、报告查看和导出。
